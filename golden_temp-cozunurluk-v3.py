...
def calculate_error_percentage(expected, found):
    """ Olması gereken yer ile bulunan yer arasındaki farkın yüzdesini hesaplar. """
    expected_x, expected_y, expected_w, expected_h = expected
    found_x, found_y, _, _ = found
    
    error_x = abs(expected_x - found_x) / expected_w * 100
    error_y = abs(expected_y - found_y) / expected_h * 100
    
    return (error_x + error_y) / 2  # Ortalama hata yüzdesi
    ...
    for image_file in image_files:
        ...
        if contours:
            ...
            if match_loc:
                ...
                cv2.rectangle(result_img, (roi[0], roi[1]), 
                              (roi[0] + roi[2], roi[1] + roi[3]), (0, 0, 255), 3)
                ...
                error_percentage = calculate_error_percentage(roi, (found_x, found_y, found_w, found_h))
